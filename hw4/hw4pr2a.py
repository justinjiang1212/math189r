...
def NLL(X, y, W, reg=0.0):
	...
	#I had to look at the solutions for this question 
	mu = sigmoid(X @ W)
...
def grad_descent(X, y, reg=0.0, lr=1e-4, eps=1e-6, max_iter=500, print_freq=20):
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		W_gradient = grad_logreg(X, y, W, reg = reg)
		W -= lr * W_grad
...
def newton_step(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	#had to look at the solutions for this one too 
	mu = sigmoid(X @ W)
